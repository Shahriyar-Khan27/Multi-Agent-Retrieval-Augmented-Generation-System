...
.main-header { padding:1.6rem 0 1.2rem 0; border-bottom:1px solid #1e2535; margin-bottom:1.5rem; display:flex; align-items:center; gap:16px; }
...
with col_main:
    # Header (SVG works fine in standalone blocks)
    st.markdown(
    ...
    if not st.session_state.messages:
        ...
        for msg in st.session_state.messages:
            ...
            if role == "user":
                st.markdown(render_user_msg(content, ts), unsafe_allow_html=True)
            else:
                st.markdown(render_ai_msg(content, agent_type, ts), unsafe_allow_html=True)
